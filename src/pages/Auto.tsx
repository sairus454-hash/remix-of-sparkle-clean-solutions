...
const Auto = () => {
  ...
  const autoItems = [
    ...
    { id: 'autoSeats', name: t.prices.items.autoSeats, price: 250, image: calcAutoSeats },
    { id: 'autoLeatherSeats', name: t.prices.items.autoLeatherSeats, price: 300, image: calcAutoLeatherSeats },
  ...
  const seoMeta = getSeoMeta('auto', language);
        ...
        <section className="py-12 sm:py-20 bg-card">
          <div className="container mx-auto px-4">
            <CircularRevealCard index={0}>
              <div className="max-w-6xl mx-auto rounded-2xl sm:rounded-3xl overflow-hidden bg-gradient-to-br from-primary/5 via-card to-fresh/5 border border-border shadow-card">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                  {/* Image */}
                  <div className="relative h-64 sm:h-80 lg:h-auto min-h-[300px] overflow-hidden">
                    <img
                      src={autoTruckVanBanner}
                      alt={t.auto?.truckBannerTitle || 'Химчистка кабин грузовиков и микроавтобусов'}
                      loading="lazy"
                      width={1280}
                      height={896}
                      className="w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-background/60 via-transparent to-transparent" />
                    <div className="absolute top-4 left-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary text-primary-foreground text-xs font-semibold shadow-glow">
                      ...
                      {t.auto?.truckBannerBadge || 'Для водителей TIR и бусов'}
                    </div>
                  </div>
                  ...
                  <div className="p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
                    <h2 className="font-serif text-2xl sm:text-3xl font-bold text-foreground mb-3 sm:mb-4">
                      {t.auto?.truckBannerTitle || 'Химчистка кабин грузовиков и микроавтобусов'}
                    </h2>
                    <p className="text-sm sm:text-base text-muted-foreground mb-6 leading-relaxed">
                      {t.auto?.truckBannerDesc}
                    </p>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-6">
                      {[
                        { icon: MapPin, title: t.auto?.truckBenefit1Title, desc: t.auto?.truckBenefit1Desc },
                        { icon: Sparkles, title: t.auto?.truckBenefit2Title, desc: t.auto?.truckBenefit2Desc },
                        { icon: Wind, title: t.auto?.truckBenefit3Title, desc: t.auto?.truckBenefit3Desc },
                        { icon: Wallet, title: t.auto?.truckBenefit4Title, desc: t.auto?.truckBenefit4Desc },
                      ].map((b, i) => (
                        <div key={i} className="flex gap-3">
                          <div className="flex-shrink-0 w-9 h-9 rounded-lg bg-gradient-hero flex items-center justify-center shadow-glow">
                            <b.icon className="w-4.5 h-4.5 text-primary-foreground" />
                          </div>
                          <div className="min-w-0">
                            <p className="font-semibold text-sm text-foreground leading-tight">{b.title}</p>
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-semibold shadow-glow hover:shadow-card-hover hover:scale-[1.02] transition-all w-full sm:w-auto self-start"
                    >
                      {t.auto?.truckBannerCta || 'Заказать химчистку кабины'}
